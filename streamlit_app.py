...
def main():
  ...
  with col1:
    student_id = st.text_input("Matrícula (RM) do estudante")
  with col2:
    secret_key = st.text_input("Chave secreta", type="password")
  usar_data_final = st.checkbox("Deseja definir uma data final específica?")
  col3, col4 = st.columns(2)
  with col3:
    data_inicial = st.date_input(
      "Data Inicial", value=datetime.now(tz=timezone('America/Sao_Paulo')))
  with col4:
    if usar_data_final:
  ...
  ocorrencia_medica = st.text_area("Detalhes da justificativa", max_chars=200)
  ...
  if st.button("Enviar"):
    ...
    try:
      ...
      response = httpx.post(
        f"https://freq-willapp.herokuapp.com/jus/{secret_key}", json=payload)
